...
export function sendOk<T>(req: Request, res: Response, data: T, meta?: Record<string, unknown>): void {
  const response = ResponseBuilder.ok(data, req.context.correlationId, meta);
  req.context.setResponse(response);
  response.send(res);
}

export function sendCreated<T>(req: Request, res: Response, data: T, meta?: Record<string, unknown>): void {
