...
export interface IFileRepository {
  ...
  updateFileAccess(fileId: string, access: {
  ...
  findFilesByUser(userId: string, options?: { 
    includeDeleted?: boolean; 
    limit?: number; 
    offset?: number 
  }): AsyncResult<File[]>;
  findFilesByOrganization(organizationId: string, options?: { 
  ...
  // FILE VARIANTS
  // ============================================
  createFileVariant(fileId: string, variant: 'thumbnail' | 'medium' | 'large', data: {
    url: string;
    width: number;
    height: number;
    size: number;
  ...
  setVirusScanResult(fileId: string, result: {
    scanned: boolean;
    result: string | null;
    scannedAt: Date;
  }): AsyncResult<boolean>;
