...
import { z } from 'zod';
import { BaseEntitySchema, BaseEntityDBSchema, SoftDeletableDBSchema, SoftDeletableSchema } from './entity.schema';
...
export const CreateCollectionTypeConfigSchema = z.object({
  typeKey: z.string().min(1).regex(/^[a-z_]+$/),
...
export const CollectionSearchSchema = z.object({
  ...
  sortBy: z.enum(['created_at', 'updated_at', 'due_at', 'priority', 'status', 'title']).default('updated_at'),
